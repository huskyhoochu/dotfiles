...
`U93`가 M.2로 확인되면 여유가 더 커진다. 2TB를 추가해 모델과 미디어를 옮기면 1TB는 시스템과 앱 데이터 전용이 되고, 위 배분에서 230GB(모델 150G + ComfyUI 80G)가 빠진다.
...
`U93`에 2TB를 추가하더라도 판단은 그대로다. 별도 볼륨으로 쓰면 btrfs로 충분하고, 두 디스크를 미러로 묶더라도 btrfs RAID1은 안정적이다.
...
3. 케이스를 열어 `U93` 슬롯이 M.2인지 확인. M.2면 2TB 추가를 검토한다
4. 모든 코드 저장소의 원격 push 상태 확인
   ```bash
   for d in ~/Documents/*/*/; do
     git -C "$d" status -sb 2>/dev/null | head -1
   done
   ```
5. 원격이 없는 로컬 전용 데이터를 찾아 외장이나 클라우드로 옮김
6. 모델 파일 목록 저장 (재다운로드 스크립트 작성)
   ```bash
