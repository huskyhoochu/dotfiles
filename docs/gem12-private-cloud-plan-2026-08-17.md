...
| 차기 장비 케이스 선정 (아래 §1-6 후보 비교) | 구매 시 |
...
| 메인보드 | ASUS ProArt X870E-Creator WiFi | PCIe 5.0 x16 ×2 (x8/x8 분기), 10GbE + 2.5GbE 내장 — 유선 전환으로 Wi-Fi 제약 소멸 |
...
| ProArt X870E-Creator WiFi | | 751,000원 |
...
| 케이스 | 아래 후보 비교 | 269,000~352,000원 |
| PSU / SSD | 재활용·이식 | 0원 |
| **합계** | | **약 390~410만 원** |

DRAM 가격 급등기라 **RAM이 보드보다 비싼 두 번째 지출 항목**이다. 64GB로 유지하는 결정이 비용 면에서도 옳다 — 96GB(48GB×2)로 올리면 이 항목만 60만 원 이상 늘어난다.

**레인 배치 — M.2_2를 반드시 비운다.** X870E는 USB4 의무 탑재로 CPU 레인이 빠듯해, ASUS 공식 스펙이 명시한다: *"PCIEX16(G5)_2는 M.2_2와 대역폭을 공유한다. M.2_2 장착 시 슬롯1은 x8, 슬롯2는 x4로 동작한다."* M.2_2를 채우면 두 번째 XTX가 지금 OCuLink 수준(x4)으로 강등된다.

```text
PCIEX16(G5)_1  ← 7900 XTX #1   (x8 Gen4)
PCIEX16(G5)_2  ← 7900 XTX #2   (x8 Gen4)   ※ M.2_2 비워야 유지
PCIEX16_3      ← 여유 (칩셋 x4 — 향후 HBA/NIC)
M.2_1 (CPU Gen5 x4)   ← 기존 1TB NVMe 이식
M.2_2 (CPU Gen5 x4)   ← 비움 (GPU 슬롯2와 공유)
M.2_3 / M.2_4 (칩셋 Gen4 x4) ← 향후 2TB 증설 자리 — GPU 레인과 무관
```

칩셋 쪽(M.2_3/4, SATA HDD, 10GbE)은 업링크 Gen4 x4를 나눠 쓰지만, NAS 부하 수준에서는 병목이 아니다.

**케이스 후보** — 요건: ATX + 두꺼운 GPU 2장 + 3.5" 다중 베이. 구매 전 실물 치수로 GPU 간섭을 확인한다. (가격: 다나와 2026-08-18)

| 후보 | 3.5" 베이 | 확장 슬롯 | 가격 | 비고 |
|---|---|---|---:|---|
| Phanteks Enthoo Pro 2 Server V2 | 최대 12 (기본 브래킷 4) | 11 | 272,160원 | 사이드 흡기 팬 브래킷이 GPU를 직접 냉각 — 듀얼 355W에 가장 유리. 풀타워로 큼 |
| Fractal Meshify 2 XL | 추가 트레이로 확장 (Type-B 2팩 ×5) | 9 | 352,000원 | 메시 전면으로 배기 양호. **storage layout에서는 GPU 길이 제한이 줄어들므로** 330mm 카드 장착 가능 여부 확인 필수 |
| Jonsbo N5 (원안) | 12 | 8 | 269,000원 | GPU ≤350mm(전면 팬 구성)이라 기존 카드(330mm)는 들어간다. NAS 챔버 구조에서 듀얼 GPU 배기 검증 필요 |

N5와 Enthoo Pro 2 Server V2는 가격이 사실상 같으므로, 듀얼 GPU 냉각(사이드 흡기)과 확장 슬롯 여유를 주는 **Enthoo Pro 2 Server V2를 우선 후보**로 둔다.

**슬롯 배치 — 기존 카드는 슬롯2로.** 기존 ASRock Phantom Gaming OC는 **330 × 140 × 57.6mm, 약 3슬롯 두께**다. x8/x8이 걸리는 두 슬롯 사이가 통상 3슬롯 간격이라, 이 카드를 슬롯1에 두면 슬롯2를 막거나 흡기 간격이 0이 된다. 따라서 **기존 카드를 아래가 트인 슬롯2에, 중고로 추가하는 카드는 2~2.5슬롯 모델(레퍼런스/MBA 287mm 등)을 골라 슬롯1에** 둔다. 중고 매물 선별 기준이 여기서 나온다 — 두께가 슬롯1 적합성을 결정한다.

**이식 방식과 성공 기준의 충돌.** NVMe를 그대로 옮기면 §11의 재구축 검증을 건너뛰게 된다. 절충: 이식으로 빠르게 가동하되, HDD를 스크래치 삼아 부트스트랩 스크립트만으로 재구축 리허설을 1회 수행해 §11을 별도로 통과시킨다. NIC 이름 변경(무선 → 유선 10GbE)에 따른 네트워크 설정 갱신은 어차피 필요하다.
